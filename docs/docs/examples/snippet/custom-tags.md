...
This page is configured using **identity**: noPii and **zero configured trackers**, instead it uses tags directly in the HTML to mimic the same data and experience as [basic-trackers](./with-trackers). It's worth noting that on these events the trackers will be initialized with the data tagged on the page so that it can provide additional `properties` into each event.
